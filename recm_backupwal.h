...
@command backup wal
@definition
Create a backup of all WAL files.
(See also {&restore wal&} command for restoring WAL files)

@option "/verbose"           "Display more details"
@option "/tag=TAG"           "Change default TAG value. (Default is 'YYYY_MM_DD_HHhMM'"
@option "/parallel=N"        "Invloke backup in parallel with N threads."
@option "/lock"              "Disable retention policy for this backup"
@option "/nochkpt"           "Do not invoke checkpoint (when your DB generate lots of WAL)"
@option "/nodelete"          "Do not delete backuped WAL after backup"
@option "/delete"            "Delete backuped WAL after backup"
@option "/switch"            "Invoke switch WAL before starting backup"
@example
@inrecm backup wal /switch /verbose 
@out recm-inf: Invoking switch WAL (73/250000F0)
@out recm-inf: Starting WAL backup of '73/26000000'
@out recm-inf: Creating piece '/Volumes/pg_backups/0001633f143c04d54a18_1_WAL.recm'
@out recm-inf: WAL file '000000240000007300000020' already backuped
@out recm-inf: WAL file '000000240000007300000021' copying
@out recm-inf: WAL file '000000240000007300000022' copying
@out recm-inf: WAL file '000000240000007300000023' copying
@out recm-inf: WAL file '000000240000007300000023.00000028.backup' copying
@out recm-inf: WAL file '000000240000007300000024' copying
@out recm-inf: WAL file '000000240000007300000025' copying
@out recm-inf: Backup Piece /Volumes/pg_backups/0001633f143c04d54a18_1_WAL.recm FileSize=83340 (Compression ratio: 99.90%)
@out recm-inf: deleting WAL '/Library/PostgreSQL/12/WALfiles/000000240000007300000020'
@out recm-inf: deleting WAL '/Library/PostgreSQL/12/WALfiles/000000240000007300000021'
@out recm-inf: Backup WAL UID '0001633f143c04d54a18' succesfully created.
@out recm-inf: Total pieces ......... : 1       
@out recm-inf: Total files backuped.. : 6       
@out recm-inf: Total files deleted... : 2        (Files aged 15 minute(s) or more)
@out recm-inf: Total Backup size .... : 83886422 (80 MB)
@inrecm
@end
*/
...
//   sprintf(WALDIR,"%s/pg_wal",varGet(GVAR_CLUPGDATA));
   ...
   // Reduce parallelism if we do not have too many files to backup
   if (walTree->entries < 100  && parallel >= 8) { parallel=2; }; 
   if (walTree->entries < 100  && parallel >= 4) { parallel=1; }; 
   if (walTree->entries < 500  && parallel >= 8) { parallel=4; }; 
   if (walTree->entries <  50  && parallel >  1) { parallel=1; }; 
