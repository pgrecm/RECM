...
#define CHANGE_BACKUPTBLS_COL_RESIZE 8
   changes=0;
   ...
   /* - 4 - alter table backup_tbl alter tab_sch type varchar(128); */
   sprintf(query,"select character_maximum_length from information_schema.columns where table_schema='%s'" 
                 " and table_name='backup_tbl' and column_name='tab_sch'",
                 varGet(GVAR_DEPUSER));
   rc=DEPOquery(query,0);
   if (rc < 0) 
   {
      ERROR(ERR_UPGRADEFAIL,"Upgrade to '%s' failed at (0xC004).\n",VERSION_12);
      DEPOqueryEnd();
      memEndModule();
      return;
   }
   rc=DEPOgetInt(0,0);
   DEPOqueryEnd();   
   if (rc != 128)                                                                // Column size is 30. upgrade can be done
   {
      changes=changes |  CHANGE_BACKUPTBLS_COL_RESIZE;
      VERBOSE("0xC004 : Done\n");
   }
   else { VERBOSE("0xC004 : skipped\n"); };
   ...
   if (optionIsSET("opt_apply") == true)
   {
      ...
      if ((changes & CHANGE_BACKUPTBLS_COL_RESIZE) == CHANGE_BACKUPTBLS_COL_RESIZE)
      {
         TRACE("Resize columns 'tab_sch,tab_nam,db_nam' from table backup_tbl to 128\n");
         sprintf(query,"alter table %s.backup_tbl alter tab_sch type varchar(128)",varGet(GVAR_DEPUSER));
         rc=DEPOquery(query,0);
         DEPOqueryEnd();   
         if (rc < 0) 
         { 
            ERROR(ERR_UPGRADEFAIL,"Upgrade to '%s' failed at (0xA004a).\n",VERSION_12);
            memEndModule();
            return;
         }
         sprintf(query,"alter table %s.backup_tbl alter tab_nam type varchar(128)",varGet(GVAR_DEPUSER));
         rc=DEPOquery(query,0);
         DEPOqueryEnd();   
         if (rc < 0) 
         { 
            ERROR(ERR_UPGRADEFAIL,"Upgrade to '%s' failed at (0xA004b).\n",VERSION_12);
            memEndModule();
            return;
         }
         
         sprintf(query,"alter table %s.backup_dbs alter db_nam  type varchar(128)",varGet(GVAR_DEPUSER));
         rc=DEPOquery(query,0);
         DEPOqueryEnd();   
         if (rc < 0) 
         { 
            ERROR(ERR_UPGRADEFAIL,"Upgrade to '%s' failed at (0xA004c).\n",VERSION_12);
            memEndModule();
            return;
         }
      }
